...
from data.FR_mos_txt import dis_list3
from models.FR_cross import IQA
from FR_config import config
from util.process import RandCrop, ToTensor, RandHorizontalFlip, Normalize
from data.FR_dataset_crop_resize import IQA_datset
...
def train_epoch(epoch, net, criterion, optimizer, scheduler, train_loader):
    ...
    for data in tqdm(train_loader):
        ...
        x_r = data['ref_img_org']
        x_r = [x.cuda() for x in x_r]
    ...
    logging.info(
        'train epoch:{} / loss:{:.4} / SRCC:{:.4} / PLCC:{:.4} / RMSE:{:.4} / KROCC:{:.4}'.format(epoch + 1, ret_loss,
                                                                                                  rho_s, rho_p, rmse,
                                                                                                  rho_k))
    print('train epoch:{} / loss:{:.4} / SRCC:{:.4} / PLCC:{:.4} / RMSE:{:.4} / KROCC:{:.4}'.format(epoch + 1, ret_loss,
                                                                                                    rho_s, rho_p, rmse,
                                                                                                    rho_k))
...
def eval_epoch(config, epoch, net, criterion, test_loader):
    with torch.no_grad():
        ...
        logging.info(
            'Epoch:{} ===== loss:{:.4} ===== SRCC:{:.4} ===== PLCC:{:.4} ===== RMSE:{:.4} ===== KROCC:{:.4}'.format(
                epoch + 1, np.mean(losses), rho_s,
                rho_p, rmse, rho_k))
        print(
            'test epoch:{}  =====  loss:{:.4}  =====  SRCC:{:.4}  =====  PLCC:{:.4} ===== RMSE:{:.4} ===== KROCC:{:.4}'
            .format(epoch + 1, np.mean(losses), rho_s, rho_p, rmse, rho_k))

        return np.mean(losses), rho_s, rho_p, rmse, rho_k
...
if __name__ == '__main__':
    ...
    logging.info('test scenes:{}'.format(test_dis))
    ...
    for epoch in range(0, config.n_epoch):
        ...
        loss_val, rho_s, rho_p, _, _ = train_epoch(epoch, net, criterion, optimizer, scheduler, train_loader)
        ...
        if (epoch + 1) % config.val_freq == 0:
            ...
            loss, rho_s, rho_p, rmse, rho_k = eval_epoch(config, epoch, net, criterion, val_loader)
            ...
            if rho_s + rho_p > best_srocc + best_plcc:
                ...
                # save weights
                if os.path.exists(config.snap_path):
                    files = os.listdir(config.snap_path)  
                    if len(files) > 0:
                        for file in files:  
                            file_path = os.path.join(config.snap_path, file) 
                            os.remove(file_path)  
                torch.save(net.state_dict(), model_save_path)
                logging.info(
                    'Saving weights and model of epoch{}, SRCC:{}, PLCC:{}, RMSE:{}, KROCC:{}'.format(epoch + 1, best_srocc, best_plcc, rmse, rho_k))
    ...
    logging.info('max SRCC: {:.4}, max PLCC: {:.4}, max KROCC: {:.4}'.format(max(sroccs), max(plccs), max(kroccs)))
